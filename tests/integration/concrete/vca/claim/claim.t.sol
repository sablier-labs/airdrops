...
contract Claim_MerkleVCA_Integration_Test is Claim_Integration_Test, MerkleVCA_Integration_Shared_Test {
    ...
    function test_RevertWhen_VestingStartTimeInFuture() external whenMerkleProofValid {
        ...
        // Move back in time so that the vesting start time is in the future.
        vm.warp({ newTimestamp: JULY_1_2024 - 1 weeks });

        // It should revert.
        vm.expectRevert(abi.encodeWithSelector(Errors.SablierMerkleVCA_ClaimNotStarted.selector));
