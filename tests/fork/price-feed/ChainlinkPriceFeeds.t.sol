...
contract ChainlinkPriceFeed_ForkTest is ChainlinkPriceFeedAddresses, Test {
    using stdJson for string;

    address internal admin = address(0xCAFE);
    SablierMerkleFactoryInstant internal factory;
    ...
    modifier initTest(string memory chainName) {
        ...
        factory = new SablierMerkleFactoryInstant(admin, getPriceFeedAddress(block.chainid));
        _;
    }
    ...
    function _test_PriceFeed(string memory tokenName) private view {
        uint256 expectedFee = tokenPricesJson.readUint(string.concat(".", tokenName));
        uint256 actualFee = factory.getMinimumFee();

        // Assert the actual fee is within 2% of the expected fee.
        uint256 tolerance = actualFee / 50;
        assertApproxEqAbs(actualFee, expectedFee, tolerance, "fee");
    }
}
